...
test('Maka publication is first, complete, bilingual and protected',()=>{
 assert.ok(model);assert.equal(models.filter(x=>x.published===true)[0].id,'6scale-woman-warrior-ca3d-pre-supported');assert.equal(model.nameZh,'玛卡·猎魂魔镰场景');assert.equal(model.nameEn,'Maka — Soul Eater by Nomnom Figures');assert.equal(model.fileCount,11);assert.equal(model.faceCount,22341254);assert.equal(model.shareCode,'tjug');assert.equal(model.sourcePolicy,undefined);assert.equal(model.usage,'树脂打印｜角色涂装｜战斗场景展示');assert.equal(model.sections.length,5);assert.equal(model.gallery.length,6);assert.match(model.authorLicense.note,/核对原始发布者/);assert.ok(model.gallery.every(x=>x.label&&!x.label.includes('AI生成')));for(const p of [model.image,...model.gallery.map(x=>x.src)])assert.ok(fs.existsSync(path.join(root,p.replace(/^\.\//,''))),p);
});
